...
// Logo fixe du cabinet (même image que celle intégrée dans public/js/pdf.js
// d'invoice-pwa) — distinct de la signature/tampon du praticien qui, elle,
// est fournie par l'appelant via le paramètre signatureB64.
...
async function generateInvoicePDF(invoice, settings, signatureB64) {
  return new Promise((resolve, reject) => {
    ...
    // ── Logo (fixe, toujours affiché, à droite du bloc date) ──
    try {
      const logoData = Buffer.from(LOGO_B64.split(',')[1], 'base64');
    ...
    if (signatureB64 && signatureB64.startsWith('data:image')) {
      try {
        ...
        const sigHeight = SIG_WIDTH * (img.height / img.width);
        doc.image(img, 45 + W/2 - SIG_WIDTH/2, y + 8, { width: SIG_WIDTH });
        y += sigHeight + 8;
      } catch(e) { y += 10; }
    } else {
      y += 10;
    ...
    // ── Bordure tableau + signature (le tout dans le même cadre) ──
    doc.rect(45, tableTopY, W, y - tableTopY + 8).stroke('#e2e8f0');
    y += 8;
    ...
    doc.font('Helvetica-Oblique').fontSize(8.5).fillColor(GRAY)
       .text('TVA non applicable, art. L. 223-3 du CIBS', 45, y);
    y += 18;
    doc.font('Helvetica-Bold').fontSize(10).fillColor(BLACK)
